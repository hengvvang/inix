...

{
  ...
  config = lib.mkIf config.mySystem.profiles.stylix.enable {
    ...
    stylix = lib.mkIf (config.mySystem.profiles.stylix.colors.enable or false) {
      # 根据选择的方案应用不同配置
      base16Scheme = 
        if config.mySystem.profiles.stylix.colors.scheme == "warm-white" then {
          # 🤍 Base16 配色方案 - 简约白色暖色调
        ...
        else if config.mySystem.profiles.stylix.colors.scheme == "cool-blue" then {
          # 🩵 Base16 配色方案 - 冷静蓝色主题
        ...
        else if config.mySystem.profiles.stylix.colors.scheme == "forest-green" then {
          # 🌿 Base16 配色方案 - 森林绿色主题
        ...
        else if config.mySystem.profiles.stylix.colors.scheme == "sunset-orange" then {
          # 🧡 Base16 配色方案 - 日落橙色主题
        ...
        else if config.mySystem.profiles.stylix.colors.scheme == "lavender-purple" then {
          # 💜 Base16 配色方案 - 薰衣草紫色主题
        ...
        else if config.mySystem.profiles.stylix.colors.scheme == "dark-elegant" then {
          # 🖤 Base16 配色方案 - 优雅深色主题
        ...
        else if config.mySystem.profiles.stylix.colors.scheme != "auto" then
          "${pkgs.base16-schemes}/share/themes/${config.mySystem.profiles.stylix.colors.scheme}.yaml"
        else null;
    };
  };
}
