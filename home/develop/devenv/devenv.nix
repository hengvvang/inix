  ...
  config = lib.mkIf (config.myHome.develop.enable && config.myHome.develop.devenv.enable) {
    ...
    home.activation.devenvInfo = lib.hm.dag.entryAfter ["writeBoundary"] (
      ...
      in ''
        ...
        echo ""
        echo "💡 使用提示:"
        echo "  - 创建项目: devenv init"
        echo "  - 进入环境: ${if config.myHome.develop.devenv.autoSwitch then "cd project-dir（自动）" else "devenv shell（手动）"}"
        echo "  - 查看文档: ~/.config/nixpkgs/home/develop/devenv/README.md"
      ''
    );
  };
}
