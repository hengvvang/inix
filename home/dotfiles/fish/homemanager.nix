...

{
  config = lib.mkIf (config.myHome.dotfiles.enable && config.myHome.dotfiles.fish.enable && config.myHome.dotfiles.fish.method == "homemanager") {
    ...
    # 相关工具配置建议
    # 可以在其他模块中配置这些工具以增强Fish体验：
    # - programs.starship.enable = true;      # 现代化提示符
    # - programs.zoxide.enable = true;        # 智能目录跳转（替代z插件）
    # - programs.fzf.enable = true;           # 模糊查找
    # - programs.bat.enable = true;           # 语法高亮的cat替代
    # - programs.eza.enable = true;           # 现代化ls替代
    # - programs.ripgrep.enable = true;       # 快速文本搜索
    # - programs.fd.enable = true;            # 快速文件查找
  };
}